...
void ahcorz(){
    ...
    vc<pii> a(n);
    rep(n) cin >> a[i].first;
    rep(n) cin >> a[i].second,a[i].second=a[i].first-a[i].second;
    sort(all(a));
    vc<pii> b;
    for(auto &[x,y]: a){
        if(sz(b)&&y>=b.back().second) continue;
        b.pb({x,y});
    }
    vi dp(1000005);
    int cur=0;
    rep(i,1,sz(dp)){
        while(cur+1<sz(b)&&b[cur+1].first<=i) cur++;
        if(b[cur].first<=i){
            dp[i]=dp[i-b[cur].second]+1;
        }
    }
    ll res=0;
    rep(_,m){
        ll x; cin >> x;
        if(x>=sz(dp)){
            ll y=(x-sz(dp)+1+b.back().second-1)/b.back().second;
            res+=y;
            x-=y*b.back().second;
        }
        if(x>=0) res+=dp[x];
